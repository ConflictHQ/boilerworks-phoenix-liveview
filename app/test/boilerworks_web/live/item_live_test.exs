defmodule BoilerworksWeb.ItemLiveTest do
  use BoilerworksWeb.ConnCase

  import Phoenix.LiveViewTest

  setup :register_and_log_in_user

  describe "Index" do
    test "lists items", %{conn: conn} do
      _item = item_fixture(%{"name" => "Test Widget"})
      {:ok, _view, html} = live(conn, ~p"/items")
      assert html =~ "Items"
      assert html =~ "Test Widget"
    end

    test "searches items", %{conn: conn} do
      item_fixture(%{"name" => "Alpha Gadget"})
      item_fixture(%{"name" => "Beta Widget"})

      {:ok, view, _html} = live(conn, ~p"/items")

      html =
        view
        |> element("form")
        |> render_change(%{"search" => "Alpha"})

      assert html =~ "Alpha Gadget"
      refute html =~ "Beta Widget"
    end
  end
end
